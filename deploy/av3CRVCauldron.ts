...
const ParametersPerChain = {
  [ChainId.Avalanche]: {
    ...
    oracle: "0xf310bD399AED40a685f159F22ef103C3E8EF6de0",
...
const deployFunction: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const oracleProxy = await ethers.getContractAt<USDCOracle>("av3CRVOracle", parameters.oracle);
  const oracleData = parameters.oracleData;

  const INTEREST_CONVERSION = 1e18 / (365.25 * 3600 * 24) / 100;
  const OPENING_CONVERSION = 1e5 / 100;

  const collateralization = 87.5 * 1e3; // 87.5% LTV
  ...
  await deployments.save("av3CRVCauldron", {
    abi: [],
    address: deployEvent?.args?.cloneAddress,
  });
  console.log("av3CRVCauldron", (await deployments.get("av3CRVCauldron")).address);
};
...
deployFunction.tags = ["av3CRVCauldron"];
