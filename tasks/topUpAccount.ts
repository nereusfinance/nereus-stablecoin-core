...
task("top-up-test-tokens", "Transfer test tokens to test account")
  ...
  .setAction(async ({ user: userAddress }, hre) => {
    ...
    await erc20TopUp(userAddress, "10000", TokenSymbol.aAvaUSDC, hre)
    await erc20TopUp(userAddress, "10000", TokenSymbol.aAvaDAI, hre)
    await erc20TopUp(userAddress, "10000", TokenSymbol.qiDAI, hre)
    await erc20TopUp(userAddress, "10000", TokenSymbol.qiUSDCn, hre)
  })
